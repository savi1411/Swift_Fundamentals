...
calculateArea(x: 0, y: 0)
calculateArea(x: 0, y: 4)
calculateArea(x: 8, y: 0)
...
func add(x: Int?, y: Int?) -> Int? {
    guard let possibleX = x,
          let possibleY = y else {
        return nil
    }
    
    // Parâmetros validados pelo guard
...
func createUser() -> User? {
    ...
    // Dados válidos - retorna usuário criado
 ...
 */
let possibleUser = createUser()
// Vamos nos certificar que o usuário foi criado
if let user = possibleUser {
    print(user)
} else {
    print("Favor verificar os dados digitados")
}
